...
for (r in 1:R) {
  ...
  res_y3 <- impute_knn(proba, y3  ~ x1 + x2 + x3 + x3 + x4 + x5 + x6, k=1)
  
  res_y1_y2_new <- impute_knn(proba, y1 + y2  ~ x1 + x2, 
                              k=floor(n^(2/(2+2)))) ## k=31 optmalne
  res_y3_new <- impute_knn(proba, y3  ~ x1 + x2 + x3 + x3 + x4 + x5 + x6, 
                           k=floor(n^(2/(2+6)))) ## k=5 - optymalne
